...
/// {CouncilVetoGovernor}. The {CouncilGovernor} needs to be able to call `propose` (and possibly
/// `execute`) on the {CouncilVetoGovernor} for the {CouncilGovernor} to work properly.
/// Using this model means the proposal will be executed by the {CouncilVetoGovernor} and not by the
...
abstract contract GovernorCouncilQueuing is Governor {
  ...
  event CouncilVetoGovernorSet(address indexed oldVetoGovernor, address indexed newVetoGovernor);

  /// @dev Veto governor requires a description to create the proposal, so we store it and pass it
  /// when queuing.
  mapping(uint256 proposalId => string) internal proposalDescriptions;

  /// @notice The council's veto governor instance.
  ...
  /// @notice Returns the current state of a proposal, taking into account the proposal's state here
  /// and on the veto governor as well.
  ...
  /// - Returns Executed if veto governor proposal is Executed (fallback for non-council execution).
  ...
  /// @notice Updates the veto governor used for proposal queuing and execution.
  /// @dev Restricted via `onlyGovernance`, with an `GovernorAdmin` override likely implied (the
  /// council ought not to change its own veto governor). @param _newCouncilVetoGovernor The new
  /// veto governor contract address.
  /// @custom:security-note It is not recommended to change the veto governor while there are other
  /// queued governance proposals.
  function setCouncilVetoGovernor(IGovernor _newCouncilVetoGovernor)
    external
    virtual
    onlyGovernance
  {
  ...
  /// @dev Checks if the {CouncilVetoGovernor}'s proposal state matches any of the allowed states.
  ...
  /// associated storage. This function, as implemented, can only cancel proposals that are in the
  /// `Pending` state on the council governor. Once a proposal has been queued (forwarded to the
  /// veto governor), it cannot be canceled through this mechanism.
  /// @param _targets Array of target addresses for the proposal calls.
  /// @param _values Array of values for the proposal calls.
  /// @param _calldatas Array of call data for the proposal calls.
  /// @param _descriptionHash Hash of the proposal description.
  ...
  /// @notice Internal function to update the veto governor.
  /// @param _newCouncilVetoGovernor The new veto governor contract.
  function _setCouncilVetoGovernor(IGovernor _newCouncilVetoGovernor) internal {
    emit CouncilVetoGovernorSet(address(councilVetoGovernor), address(_newCouncilVetoGovernor));
