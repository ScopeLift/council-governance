...
} from "src/interfaces/MarketAdminPermissionCheckerInterface.sol";
...
contract BaseBridgeReceiver {
  ...
  event Initialized(
    ...
    address indexed marketAdminPermissionChecker
  );
  ...
  modifier onlyOwnerOrMarketAdmin(address _caller) {
    if (_caller != govTimelock) marketAdminPermissionChecker.checkUpdatePermission(_caller);
    _;
  }
  ...
  /// @notice MarketAdminPermissionChecker contract which is used to check if the caller has
   ...
   */
  function initialize(
    address _govTimelock,
    address _localTimelock,
    address _marketAdminPermissionChecker
  ) external {
    ...
    marketAdminPermissionChecker =
      MarketAdminPermissionCheckerInterface(_marketAdminPermissionChecker);
    initialized = true;
   ...
   */
  function processMessage(address rootMessageSender, bytes calldata data)
    internal
    onlyOwnerOrMarketAdmin(rootMessageSender)
  {
